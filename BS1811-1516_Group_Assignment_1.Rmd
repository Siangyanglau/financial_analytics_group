...
# and thus determine the final payoff of each path and 
...
RF_GBM_di<-RF_GBM[,ncol(RF_GBM)]

if(any(RF_GBM > H)){
  payoff_di <-as.list(rep(0, 10000))
  
}else{
  payoff_di <-pmax (RF_GBM[,ncol(RF_GBM)] - strike, 0)
}

payoff_di<-as.numeric(payoff_di)

#Exotic Option - down-and-out
RF_GBM_do<-RF_GBM[,ncol(RF_GBM)]


if(any(RF_GBM > H)){
  payoff_do <- pmax (RF_GBM[,ncol(RF_GBM)] - strike, 0)
}else{
  payoff_do <- 0
}
...
###################################################################
# Step 3 - Discount all future payoff distribution of values to t0.
###################################################################
#Vanilla Opiton
...
################################################################
# Step 4 - Calculate the mean of discounted payoff distribution.
################################################################
#Vanilla Opiton
